...
class _DataImportDialogState extends State<DataImportDialog> {
  ...
  void _startInstall() async {
    final novelTitle = await NovelDataServices.getNovelTitle(widget.path);
    if (!mounted) return;
    if (novelTitle == null) {
      showDialogMessage(context, 'novel title not found!');
      return;
    }
    final saveDir = Directory('${PathUtil.getSourcePath()}/$novelTitle');
